...
export const AuthorizedComponent = ({children,modalOption,retrievePassword}) => {
    ...
    const AccessQuery=useMutation({
        ...
        onSuccess:(data)=>{
            ...
            // queryClient.invalidateQueries('decryptedData')
  ...
  return (
   ...
   modal!==ModalOptions.NONE?modal===modalOption?
       ...
       { !decryptedDataQuery.data ? <div>
        <InputForm  setModal={setModal} modal={modal}   modalOption={modalOption}  query={AccessQuery}/>
    </div>  :children
...
const InputForm=({query,setModal,modal,modalOption  })=>{
    ...
    return(
        <div    className='lg:w-[30vw]  w-[80vw] h-[40vh]    p-3 bg-white    relative    rounded-md  flex    flex-col'  ref={containerRef}  >
...
<input type="text" placeholder='Pin' {...register("pin",{
